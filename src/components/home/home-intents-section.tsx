...
function IntentCard({ intent }: { intent: HomeIntent }) {
  const className = cn(homeCardClass, "group block p-3 transition-colors")

  if (intent.link.kind === "external") {
    return (
      <a
        href={intent.link.href}
        target="_blank"
        rel="noopener noreferrer"
        className={className}
      >
        <IntentCardContent intent={intent} />
      </a>
    )
  }
